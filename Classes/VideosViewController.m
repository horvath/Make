    ...
    for (CXMLElement *resultElement in resultNodes) {
        ...
        for(counter = 0; counter < [resultElement childCount]; counter++) {
            ...
            // Add each field to the blogItem Dictionary with the node name as key and node value as the value
				...
				//NSLog(@"FUCKING WHACKNESS: %@", [resultElement childCount]);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	storyDesc.text = [[blogEntries objectAtIndex: blogEntryIndex] objectForKey: @"description"];
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if([blogEntries count] == 0) {
        ...
        NSString *blogAddress = @"http://blog.makezine.com/archive/make_podcast/index.xml";
